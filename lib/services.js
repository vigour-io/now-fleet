...
module.exports = class Services {
  ...
  discoverAll (pkg, delay) {
    if (!pkg._registry_host || !pkg._services || (!pkg._env && pkg._env !== '')) { return Promise.resolve({}) }
  ...
  deploy (service) {
    ...
    return (root ? Promise.resolve() : command.run(`npm install ${service.name}@${service.version}`, this.rootDir))
      .then(() => {
        ...
        return root ? true : command.run('npm install --production', dir)
      })
      .then(() => root ? true : command.run('now -N', dir))
