...
module.exports = class Services {
  ...
  deployAll (dir, env) {
    ...
    return registry.getList()
      ...
      // deployment of root service will be done separately
      .then(() => this.servicesFlat.shift())
      // all the others in the tree
      .then(() => Promise.all(this.servicesFlat.map(this.deploy.bind(this))))
  }
  discoverAll (pkg, delay) {
    if (!pkg._registry_host || !pkg._services || !pkg._env) { return Promise.resolve({}) }
  ...
  deploy (service) {
    ...
    const dir = path.join(this.rootDir, 'node_modules', service.name)

    return command.run(`npm install ${service.name}@${service.version}`, this.rootDir)
      .then(() => {
        var _services = {}
