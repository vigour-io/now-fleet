...
exports.deployAll = (dir, env) => {
  ...
  return registry.getList()
    ...
    .then(() => {
      ...
      // prepare install object for pnpm
      var install = {}
      data.servicesFlat.filter(s => s.deploy).forEach(s => {
        install[s.name] = s.version
      })

      if (Object.keys(install).length) {
        // pnpm install all together
        console.log(`NowFleet: pnpm installing ${JSON.stringify(install)}...`)
        return pnpm.installPkgs(install, {
          cwd: dir, save: false, quiet: true
        })
      }
    })
    ...
    .then(() => {
      ...
      preparePkg(rootService, data.rootDir)

      // cleanup pnpm store
      return command.run(`rm -r ${path.join(dir, 'node_modules', '.store')}`)
    })
}
