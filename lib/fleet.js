...
exports.deployAll = (dir, env) => {
  ...
  return registry.getList()
    ...
    .then(() => {
      ...
      // prepare package for the root service (app)
      preparePkg(data.servicesFlat.shift(), data.rootDir)
    })
...
function deploy (service) {
  ...
  console.log(`NowFleet: npm installing ${service.name}@${service.version}`)
  return command.run(`npm install ${service.name}@${service.version}`, data.rootDir)
    .then(() => {
      preparePkg(service, dir)

      fs.writeFileSync(path.join(dir, '.npmrc'), `//registry.npmjs.org/:_authToken=${process.env.NPM_TOKEN}`)
      const env = data.env
        .split('&').map(pair => `-e ${pair}`)
        .concat(`-e REGISTRY_HOST=${process.env.REGISTRY_HOST}`)
        .join(' ')

      console.log(`NowFleet: deploying ${service.name}@${service.version} with ${data.env}`)
      return command.run(`now -N ${env}`, dir)
    })
