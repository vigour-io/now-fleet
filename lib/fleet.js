...
exports.getServices = (pkg, dir, env) => {
  if (!process.env.REGISTRY_HOST) {
    console.log('env.REGISTRY_HOST is not in place!')
    process.exit(1)
  }
  ...
  if (pkg._services) {
    ...
    data.dir = dir && dir.constructor === String ? dir : process.cwd()
...
function deploy (service) {
  ...
  return new Promise((resolve, reject) => {
    const sdep = now.deployment(process.env.NOW_TOKEN)
      .deploy(dir, env)
      .on('deployed', () => {
        console.log(`NowFleet: ${service.name}@${service.version} deployed:`, sdep.get('url').compute(), 'waiting until ready...')
      })
      .on('ready', () => {
        console.log(`NowFleet: ${service.name}@${service.version} is ready.`)
        service.lastUrl = sdep.get('url').compute().replace(/^https:\/\//, '')
        service.deploy = false
        resolve()
        sdep.set(null)
      })
