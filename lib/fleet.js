...
exports.deployAll = (dir, env) => {
  ...
  return registry.getList()
    ...
    .then(() => {
      ...
      // take root service out of flat array
    ...
    .then(() => {
      ...
      return new Promise((resolve, reject) => {
        const sdep = now.deploy(dir, env, process.env.NOW_TOKEN)
          .on('deployed', () => {
            console.log(`NowFleet: ${service.name}@${service.version} deployed, waiting until ready...`)
          })
          .on('ready', () => {
            console.log(`NowFleet: ${service.name}@${service.version} is ready:`, sdep.url.compute())
